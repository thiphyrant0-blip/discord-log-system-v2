...
function getSourceGuild() {
    return client.guilds.cache.get(SOURCE_GUILD_ID);
}
...
function getLogGuild() {
    return client.guilds.cache.get(LOG_GUILD_ID);
}
...
function findLogChannel(key) {
    ...
    for (const [categoryName, channels] of Object.entries(LOG_STRUCTURE)) {
...
async function sendLog({
    ...
    try {
        ...
        const channel = findLogChannel(key);

        if (!channel) {
            console.log(`⚠️ ไม่พบห้อง Log: ${key}`);
            return;
        }
...
async function getAuditExecutor(
    ...
    try {
        ...
        const entry = logs.entries.find(log => {
            ...
            const age = now - log.createdTimestamp;
        ...
        return {
            user: entry.executor,
            reason: entry.reason || "ไม่ได้ระบุเหตุผล",
            entry
        };
...
function auditFields(audit) {
    ...
    return [
        {
            name: "👮 ผู้ดำเนินการ",
            value: userInfo(audit.user),
            inline: true
        },
        {
            name: "📝 เหตุผล",
            value: safeText(audit.reason, 300),
            inline: true
        }
    ];
}
...
async function setupLogSystem(guild) {
    ...
    console.log("🔧 กำลังตรวจสอบระบบ Log...");
    ...
    console.log("====================================");
    console.log("✅ LOG SYSTEM V2 พร้อมใช้งาน");
    console.log("====================================");
}
...
client.once("ready", async () => {
    ...
    console.log("🚀 MEAOW LOG SYSTEM V2");
    console.log("====================================");

    const sourceGuild = getSourceGuild();
    const logGuild = getLogGuild();
    ...
    console.log(`📋 LOG: ${logGuild.name}`);

    await setupLogSystem(logGuild);

    await sendLog({
        key: "bot_status",
        title: "🟢 LOG SYSTEM V2 ONLINE",
        description:
...
client.on("guildMemberAdd", async member => {
    ...
    if (member.guild.id !== SOURCE_GUILD_ID) return;

    await sendLog({
        key: "member_join",
        title: "📥 สมาชิกเข้าเซิร์ฟเวอร์",
        description:
...
client.on("guildMemberRemove", async member => {
    ...
    const auditKick = await getAuditExecutor(
        member.guild,
...
client.on("messageDelete", async message => {
    ...
    if (message.guild.id !== SOURCE_GUILD_ID) return;
...
client.on("messageUpdate", async (oldMessage, newMessage) => {
    ...
    if (oldMessage.guild.id !== SOURCE_GUILD_ID) {
        return;
    }
...
client.on("voiceStateUpdate", async (oldState, newState) => {
    ...
    if (oldState.guild.id !== SOURCE_GUILD_ID) {
        return;
    }
...
client.on("guildMemberUpdate", async (oldMember, newMember) => {
    ...
    if (oldMember.guild.id !== SOURCE_GUILD_ID) {
        return;
    }
    ...
    for (const role of addedRoles.values()) {
        ...
        await sendLog({
            key: "role_update_member",
            title: "🎭 ใส่ยศ-ถอดยศรวม",
            description:
                `${newMember} ได้รับ Role`,
            color: COLORS.green,
            fields: [
...
client.on("guildBanAdd", async ban => {
    ...
    if (ban.guild.id !== SOURCE_GUILD_ID) {
        return;
    }
...
client.on("roleCreate", async role => {
    ...
    if (role.guild.id !== SOURCE_GUILD_ID) {
        return;
    }
...
client.on("roleUpdate", async (oldRole, newRole) => {
    ...
    if (oldRole.guild.id !== SOURCE_GUILD_ID) {
        return;
    }
    ...
    if (
        ...
        changes.push("Permissions เปลี่ยน");
    }
...
client.on("channelCreate", async channel => {
    ...
    if (channel.guild.id !== SOURCE_GUILD_ID) {
        return;
    }
...
client.on("channelUpdate", async (oldChannel, newChannel) => {
    ...
    if (oldChannel.guild.id !== SOURCE_GUILD_ID) {
        return;
    }
...
client.on("guildUpdate", async (oldGuild, newGuild) => {
    ...
    if (oldGuild.id !== SOURCE_GUILD_ID) {
        return;
    }
    ...
    if (oldGuild.icon !== newGuild.icon) {
        changes.push("ไอคอนเซิร์ฟเวอร์เปลี่ยน");
    }
...
client.on("stickerCreate", async sticker => {
    ...
    if (sticker.guild.id !== SOURCE_GUILD_ID) {
        return;
    }

    await sendLog({
        key: "sticker_create",
        title: "🏷️ เพิ่มสติกเกอร์",
        description:
            `มีการเพิ่ม Sticker`,
        color: COLORS.green,
        fields: [
...
client.on("inviteCreate", async invite => {
    ...
    if (invite.guild.id !== SOURCE_GUILD_ID) {
        return;
    }
...
client.on("messageCreate", async message => {
    ...
    if (message.guild.id !== LOG_GUILD_ID) {
        return;
    }
    ...
    try {
        ...
        await setupLogSystem(message.guild);
